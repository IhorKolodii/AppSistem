...
def vklogindo():
    ...
    m.update(hashstring)    
    if vk_cookies['sig']==m.hexdigest():
        user = AppUsers.query(AppUsers.vk_id==vk_cookies['mid']).fetch()
        if user:
            response.set_cookie('uid', user[0].local_id)
            redirect('/')
        else:
            ...
            if last_user_registered:
                ...
                new_user.local_id = '1'
            new_user.vk_id = vk_cookies['mid']
            new_user.login_type = 2
            
            vk_data_request = vk_user_get_url + '?' + urllib.urlencode({'uids': vk_cookies['mid'], 'fields': 'uid,first_name,last_name,nickname'})
            ...
            new_user.nick = vk_firstname + ' ' + vk_nickname
            new_user.isadmin = False
            new_user.put()
            response.set_cookie('uid', new_user.local_id)
            redirect('/')
